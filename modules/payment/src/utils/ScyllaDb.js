...
async function getItem(tableName, key) { const result = await getClient().send(new GetItemCommand({ TableName: tableName, Key: marshallItem(key) })); if (!result.Item) return null; return unmarshallItem(result.Item); }
...
async function _reset() { 
  const paymentTables = ['paymentGateway_sessions', 'paymentGateway_transactions', 'paymentGateway_schedules', 'paymentGateway_tokens', 'paymentGateway_webhooks'];
  for (const tableName of paymentTables) {
    try {
      const items = await scan(tableName);
      for (const item of items) {
        const key = tableName.includes('sessions') ? { pk: item.pk, sk: item.sk } : tableName.includes('transactions') ? { pk: item.pk, sk: item.sk } : tableName.includes('schedules') ? { pk: item.pk, sk: item.sk } : tableName.includes('tokens') ? { pk: item.pk, sk: item.sk } : { pk: item.pk, sk: item.sk };
...
async function execute(_query, _params) {
  try {
    await ping();
    return { rows: [] };
  } catch (e) {
