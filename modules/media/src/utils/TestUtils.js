 ...
 */
async function runTest(title, testFunction) {
    console.log(`\n--- TEST ${counters.testCounter++}: ${title} ---`);
    try {
        await testFunction();
        counters.passedTestCounter++;
        console.log(`[TEST PASSED] ${title}`);
    } catch (error) {
        counters.failedTestCounter++;
 ...
 */
async function assertThrows(asyncFn, expectedErrorMsg, message) {
    let error;
    try {
        await asyncFn();
    } catch (e) {
