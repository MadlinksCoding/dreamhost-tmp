...
class ScyllaDb {
  ...
  static async request(
    ...
    while (true) {
      ...
      console.log(`ScyllaDb.request: attempt #${attempt}`);
      const now = new Date();
      const amzDate = now
        .toISOString()
        .replace(/[:-]|\.\d{3}/g, "")
      ...
      try {
        ...
        console.error(`ScyllaDb ${target} failed: ${netErr.message}`);
        // console.error(ScyllaDb.#errors);
        throw netErr;
      }
    }
  }
  ...
  static async query(table, keyConditionExpr, exprVals, options = {}) {
    let payload;
    if (typeof keyConditionExpr === 'object' && keyConditionExpr !== null) {
      // If keyConditionExpr is an object, treat it as the full payload
      payload = { TableName: table, ...keyConditionExpr, ...options };
    } else {
      // Original signature
   ...
   */
  static async scanPaginated(table, options = {}) {
    if (!table) {
      throw new TypeError("scanPaginated: table name is required");
    }
    ...
    // Handle nextToken -> ExclusiveStartKey
    if (options.nextToken) {
        try {
            payload.ExclusiveStartKey = JSON.parse(Buffer.from(options.nextToken, 'base64').toString('utf8'));
        } catch (e) {
    ...
    const items = (resp.Items ?? []).map((it) => ScyllaDb.unmarshalItem(it));
    
    let nextToken = null;
    if (resp.LastEvaluatedKey) {
        nextToken = Buffer.from(JSON.stringify(resp.LastEvaluatedKey)).toString('base64');
    }
